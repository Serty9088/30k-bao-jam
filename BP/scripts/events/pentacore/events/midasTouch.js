 ...
 */
const playersWithMidasTouch = new WeakMap();
...
LuckyEventType.register({
    ...
    callback: (event) => {
        ...
        playersWithMidasTouch.set(player, system.currentTick);
        world.sendMessage(`${player.name} has the §gMidas Touch!`);

        function particleAndSound(loc) {
            dim.spawnParticle("minecraft:ice_evaporation_emitter", loc);
            dim.playSound("dig.stone", loc, { pitch: 1.2 });
        }
        ...
        const hitEntity = world.afterEvents.entityHitEntity.subscribe(
