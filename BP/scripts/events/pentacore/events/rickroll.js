    ...
    callback: async (event) => {
        ...
        function exit() {
            ...
            if (player?.isValid) {
                 ...
                 */
                const originalLocation = JSON.parse(String(player.getDynamicProperty("bao_30k_pentacore:rickroll;player_pos")));
                /**
                 * @type {GameMode}
                 */
                // @ts-expect-error
                const originalGameMode = player.getDynamicProperty("bao_30k_pentacore:rickroll;player_game_mode");
...
world.afterEvents.playerSpawn.subscribe((event) => {
    if (event.player.getDynamicProperty("bao_30k_pentacore:rickroll;player_pos")) {
        /**
         * @type {import("@minecraft/server").Vector3 & { dimension: string }}
         */
        const originalLocation = JSON.parse(String(event.player.getDynamicProperty("bao_30k_pentacore:rickroll;player_pos")));
        /**
         * @type {GameMode}
         */
        // @ts-expect-error
        const originalGameMode = event.player.getDynamicProperty("bao_30k_pentacore:rickroll;player_game_mode");
...
world.afterEvents.worldLoad.subscribe(() => {
    for (const player of world.getAllPlayers()) {
        if (player.getDynamicProperty("bao_30k_pentacore:rickroll;player_pos")) {
            /**
             * @type {import("@minecraft/server").Vector3 & { dimension: string }}
             */
            const originalLocation = JSON.parse(String(player.getDynamicProperty("bao_30k_pentacore:rickroll;player_pos")));
