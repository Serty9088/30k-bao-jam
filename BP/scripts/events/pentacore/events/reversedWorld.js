import { BlockPermutation, BlockVolume, StructureSaveMode, system, world } from "@minecraft/server";
    ...
    callback: (async event => {
        ...
        for (let i = -size+0; i <= size; i++) {
            ...
            const volume = new BlockVolume(
                f.Vector.sum(f.Vector.subtract(blockLocation, offset), { y: i }),
                f.Vector.sum(blockLocation, { ...offset, y: i })
            );
            ...
            for (const location of volume.getBlockLocationIterator()) {
                if (f.Geo.distance(location, blockLocation) > radius) {
        ...
        const placeLocation = f.Vector.subtract(blockLocation, offset);
        for (let i = -size+0; i <= size; i++) {
